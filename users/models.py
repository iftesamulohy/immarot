...
class CustomGroupManager(models.Manager):
    def create_custom_group(self, title):
        # Custom logic to create and save a group with a generated name
        group = self.create(title=title)
        group.name = f"{title}_{group.id}"  # Generate the name dynamically
        group.save()
        return group
